...
from hummingbot.market.ripio.ripio_api_user_stream_data_source import RipioAPIUserStreamDataSource
from hummingbot.market.ripio.ripio_auth import RipioAuth


class RipioUserStreamTracker(UserStreamTracker):
    ...
    def __init__(
        ...
        ripio_auth: Optional[RipioAuth] = None,
        trading_pairs: Optional[List[str]] = [],
    ):
        super().__init__(data_source_type=data_source_type)
        self._ripio_auth: RipioAuth = ripio_auth
    ...
    def data_source(self) -> UserStreamTrackerDataSource:
        if not self._data_source:
            if self._data_source_type is UserStreamTrackerDataSourceType.EXCHANGE_API:
                self._data_source = RipioAPIUserStreamDataSource(
                    ripio_auth=self._ripio_auth, trading_pairs=self._trading_pairs
                )
            else:
